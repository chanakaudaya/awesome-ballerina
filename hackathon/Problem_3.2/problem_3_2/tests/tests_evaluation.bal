...
function testGetTopXBillionairesEval(string[] countries, int 'limit, string[] expected) {
    ...
    test:assertTrue(actual == expected);
}

function validDataEval() returns map<[string[], int, string[]]> {
    return {
        "case1": [
            ["United States"],
            3,
