...
import problem_7_2.hub as \$testprojzonehub;
...
function testNumberOfEntries() returns error? {
    ...
    lock {
        if \$testReadArray.length() == expectedLength {
            return;
        }
    }
...
function testNotProcessingFeatureRequestEventNotifications() returns error? {
...
function testNotSubscibingForFeatureRequestEventNotifications() {
    websubhub:Subscription[]? featureRequestTopicSubscribers = 
        \$testprojzonehub:retrieveAvailableSubscriptions("http://projzone.com/gofigure/Connectors/events/featurerequests.json");
    test:assertTrue(featureRequestTopicSubscribers is ());
}
