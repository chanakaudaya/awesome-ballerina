...
@test:Config {
    dataProvider: validInsertionTestDataEval,
    groups: ["evaluation"]
}
function addEmployeeTestEval(string dbFilePath, string name, string city, string department, int age, int expected) returns error? {
    test:assertTrue(addEmployee(dbFilePath, name, city, department, age) > expected);
}

@test:Config {
    dataProvider: invalidInsertionTestDataEval,
    groups: ["evaluation"]
}
function addEmployeeTestInvalidEval(string dbFilePath, string name, string city, string department, int age, int expected) returns error? {
    test:assertTrue(addEmployee(dbFilePath, name, city, department, age) == expected);
}
...
function checkExactInsertValueEval() returns error? {
    ...
    test:assertTrue(employeeNameInDB == "PeterTest125");
}
